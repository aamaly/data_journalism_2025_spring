...
**Answer** This is important because it is just showing two columns, not all nine columns. Additionally, this new variable is crucial to differentiate the narrowed down columns with the 2025 journalism courses, versus the all nine columns showing everything. It's necessary to keep them seperate to have the aggregated full data, but also, in order to see just a narrow list -- a new varibale should be made. 
...
**Answer:** Instead of the under the column name "expenditure_date," it reads "date" rather than "chr," which shows that the mutate code (lubridate) was able to change the the data to read as a numerical date. However, instead of the slashes for the data format, now its dashes and it uses a different format for dates: year-month-day not month/day/year, at least for this one.  
...
**Answer** The month column shows the first day of each month in a year that had the highest total amount of expenditures. So, the "total_amount" column is descending as shown in the code, as the month's first date is shown as which month has the most expenditure. As already state, June 2022 has the most expenditures by far in the data shown. 
...
**Answer** Based on the information provided when running this code, I think it is giving the general foundation of how many rows and columns will be in the dataset. Based on the first codeblock from this task, there are six more columns, so a total of 14 columns, and now there are 97,877 rows instead of 24. Additionally, there are more column specifics than the original dataset. 
...
**Answer** I only saw the code in the console. But based on the codeblocks, I believe the latter code would be arranged from greatest to least, while the percentage is still there. This is because of the last code line that asks to arrange the percentages in a descending order.   
...
**Answer** This is a more interesting answer because it shows all the other candidates votes from each county, but you can't immediately see the percentage of Moore's wins in each county easily. However, the code was pretty simple and the percentages of Moore's wins are shown in an ascending order in the "pct_moore" column. Additionally, it is grouped by the county, so it seems to have been that the least amount of Moore's votes came from Garrett County, and some of the other red counties/right leaning.   
...
**Answer** 1. str_to_lower() -- converts a string (or vector of strings) to all lowercase. 2. str_to_sentence() -- converts a string so that only the first letter of the sentence is capitalized; the rest is lowercased (i.e. sentence case). Used ChatGPT, just asking it, "Search the Internet for tidyverse functions similar to str_to_upper. Name at least two and describe what they do. If you use ChatGPT for this, include the question you submitted." 
...
**Answer** I think the mutate statement is detecting when there is a full state name included in the address column, and making it abbreviated in the state column. If there is not a state or full state name included, it would be described as "NA." 
...
**Answer** Yes, because it shows, simply, the state and expenditures used -- in two columns and ten rows. However, I think I would need to know a bit more information regarding the demographics, and how much exact money used by each business or individual within Maryland to get the total.  
...
1.  Which courses have seen the largest percentage increase in enrollment between Spring 2022 and Spring 2023? (use mutate to calculate percentage change.)

2.  What is the average course enrollment by department for 400-level courses offered in Spring 2023?

3.  In which month of fiscal year 2023 did the Maryland spend the most on health-related programs? 
