...
**Task** Load the five CSV files. Do they have the same column structure? **Answer** These datasets have the same column names, however, some of the other political parties after Democrat or Republic are listed differently, probably by the greater number of votes in one category compared to another. 
...
**Task** Load the Maryland 2020 county population data **Answer** One of the columns in the county_voters_2020 dataset, is called "county." So it is not unique to use, and may get confusing. 
...
**Task** Create a new dataframe with population and voter data joined together. What does the output message below say about the join? **Answer** The message states, "Joining with 'by = join_by(COUNTY)'" -- I think it's joined by county now. 
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer** Prince George's County has 60.9% of a Black population that's 18-years-old or over and 78.2% of the county is a Democrat. 
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer** Now, the highest Asian population that is 18 and over is Howard County. And Montgomery County follows it. However, the highest percentage of Democrats remain in Prince George's County and Baltimore City, but have a relatively small Asian population over the age of 18. This doesn't match as much as the previous code block, and the matchup of which county's had the highest Democrat population and specific race that is over the age of 18. 
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N? column? **Answer** It's a dbl, meaning a numerical item/column. 
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N now? **Answer** Now it's a character, meaning the zipcode will be text instead of a number. 
