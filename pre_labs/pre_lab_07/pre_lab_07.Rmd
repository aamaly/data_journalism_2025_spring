...
**Task** Run the following codeblock to calculate the percentage change and put the state with the largest percentage growth below. **Answer** The state with the largest percentage growth is Nevade with a 35% increase in population between 2000 and 2010. 
...
**Task** Run the following codeblock to find the Maryland county with the highest median household income and put the answer below. **Answer** Howard County has the highest median household income, with an estimated $129,549. 
...
**Task** Run the following codeblock to create a chart showing the margin of error for each county. The current headline isn't very descriptive of the data. What should it be? **Answer** It should be something along the lines of "Estimated median household incomes by county in Maryland with margin of errors."
...
Is the difference between Calvert and Montgomery significant? **Answer** No, the difference between Calvert and Montgomery are not statistically significant because the bars overlap, and it is within the margin of error. 

Is the difference between Howard and everyone else significant? **Answer** I think the differences are statistically significant because there are no overlaps between the bars. 
...
**Task** Run the following codeblock to join the data, rename the population estimates and then calculate the values we need. Notice how we select *after* we do the initial calculations so we only see the columns we're interested in (and drop the ones we don't want to see). Also note how the select function has the minus sign (-) in front of some columns? That means we don't want to see them in our result. Which jurisdictions saw the largest increases in median income? Did any counties see their median incomes fall from 2010-2021? **Answer** Baltimore City -- despite starting with a low median income in 2010 -- saw the largest increase statewide, about a 37% increase, compared to the other counties. Additionally, Allegany and Calvert County had a large increase in median income (35% and 32% increase, respectively). Likewise, Frederick County and Anne Arundel County had about a 29% increase from 2010 to 2021. No counties experienced a decline in nominal median household income between 2010 and 2021 --- every jurisdiction's median rose in dollar terms. However, when adjusting for inflation (roughly 28% between 2010 and 2021), the small increases in Caroline and Somerset counties mean that real household incomes likely declined, showing that residents there lost purchasing power even as nominal incomes rose slightly. 
...
**Task** Let's get ZCTA household income data from the ACS using tidycensus for each zip code in `md_high_schools`. Run the following codeblock to retrieve household income data from 2019 - the latest year for which we have it for all Maryland ZCTAs - first, then join them to `md_high_schools` using the GEOID column. We'll also create min and max values and drop any rows where we don't have income estimates. What's one question we could ask of this data now that we have enrollment percentages and household income for the high school's zip code? What might make the answer harder to interpret? **Answer** We could ask whether schools in higher-income areas tend to have higher or lower enrollment percentages than those in lower-income areas. Basically, is there a link between the wealth of a school's surrounding community and how many students attend that school? Figuring that out, though, isn't so straightforward. ZIP codes don't always line up neatly with school boundaries, so a high school might pull students from several different neighborhoods with very different income levels. Plus, the income numbers come from community-wide estimates, not from the actual families of students at the school, and those estimates have built-in uncertainty. On top of that, things like school choice programs, magnet options and transporation can all affect where students end up going. So while this data can give us some clues about patterns between income and enrollment, the full story is definitley more complicated and harder to interpret. 
...
**Task** Let's look at the average household income for high schools that have 60% or more graduates immediately enrolling in college, and then look at those with less than 40%. Write a sentence describing the results. **Answer** High schools where 60% or more graduates immediately enroll in college are located in zip codes with a much higher average household income (111,448 dollars) compared with schools where fewer than 40% of graduates go straight to college (69,252 dollars), representing a strong link between local income levels and college-going rates. 
...
**Task** What is another question you could ask of this data? **Answer** How does the size of the graduating class relate to the percentage of students immediately enrolling in college? Does higher graduating class mean more students enrolling in college? Or vice versa? 
