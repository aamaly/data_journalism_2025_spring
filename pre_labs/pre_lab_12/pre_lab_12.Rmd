...
**Task** Run the following code in the gray-colored codeblock below to extract all tables in the HTML and save it as a dataframe. What do you notice about the columns? **Answer** The date column is categorized as a character instead of a numerical, and there is no hyperlink to the actual press releases, it's just the titles. Additionally, it's just two columns with a capitalized word, sigifying the "Date" and "News Releases," which isn't necessarily clean data/columns. 
...
**Task** Run the following code to extract the table we want, clean up the names and change the date column to an actual date. Look at the dataframe; what other information about these releases might you want to add? **Answer** Potentially a link or URL to the news release, if that can't be done maybe the lede or summarization of the release. Category/topic, media contact information, geographical location, source/issuing office and type of release (alert, standard news release, etc) would be great information to add. 
...
tags there are in the HTML source? **Answer** I don't see any <ul> tags, but I see 20 <li> tags, possibly 94 because of the note at the top. 
...
tags. Describe the output: **Answer** Now, we see exactly all 94 texts that would be on the website page of the press releases. So, each <li> tag was the actual text on the webstie page. In this output, it is formatted as the HTML text instead of the code and links. 
...
**Task** Run the following code in the gray-colored codeblock below to make a dataframe. Describe the output: **Answer** The HTML text listed previously in 94 items is now a dataframe that is topped with a column called "value." It's not just a long list with quotations or parenthesized numbers. However, it's not totally clean data as there is inconsistent lettering and punctuation. 
