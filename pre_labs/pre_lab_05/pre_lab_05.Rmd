...
**Task** Export the CSV file to your pre_lab_05 folder and read it into R by running the following codeblock. What problems do you see with the data? **Answer** Since there are no columns, the code "clean_names" could not clean any of the column names. The dataframe shows Maryland as the column headers of the data, and includes its stats as the other column names, which is all inaccurate. 
...
**Task** Load the data without headers by running the codeblock below. What problems remain? **Answer** I still cannot clean the column headers because it seems like it's listed under X and a number. And again, the "clean_names()" isn't running. So, I had to # it out. 
...
**Task** Run the codeblock below. What else do we need to do? **Answer** Remove the periods in the jurisdiction column.

```{r}
fentanyl_deaths <- read_csv(
  "tabula-Annual_2020_Drug_Intox_Report.csv", 
  col_names = FALSE, 
  skip = 0, 
