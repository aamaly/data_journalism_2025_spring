...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer** There are duplicates of grantees because they have almost identical names. So, this is preventing proper grouping of grantees and how many grants/loans recieved. For example: "WALTER P CARTER CHILD CARE CENTER"	and 		
"WALTER P CARTER CHILD CARE CENTER INC." These are almost identical, and probably the same center, but one has "INC." at the end and the other does not. 
...
**Task** Run the following code to make a copy of the Grantee field called grantee_clean, then use the `key_collision_merge` function from refinr to attempt to standardize similar values. Store it as a new object called cleaned_md_grant_loans Click the black arrow on the table to examine the Grantee and grantee_clean columns. Looking at the results on the first 3 pages only (first 30 rows), name two changes the refinr package made, by comparing Grantee to grantee_clean. **Answer** For some of the duplicates, the refinr took out punctuation or made the letters all uppercase. (ex: Grantee: "FAIR HILL FARM, INC." and grantee_clean: "FAIR HILL FARM INC"). There's also one where the refinr package took out the Inc all together, and did not change all the letters to uppercase (ex: Grantee:" "Melwood Horticultural Training Center, Inc." and grantee_clean: "Melwood Horticultural Training Center"). 
...
**Task** Run the following code to group by grantee_clean and Grantee, then filter for recipients that start with W. Name three changes the refinr function made in the first 30 results. **Answer** 1. It puts the same exact name for any grantee that are duplicates.Basically, this is standardizing slightly different spellings or formatting into a single grantee_clean. 2. Made WASHINGTON ADVENTIST UNIVERSITY into title case instead of all uppercase in the grantee_clean. 3. Removed some of the suffixes like "INC.", "CO.", or "LLC" from company names so the cleaned version is simpler. But not all of them eliminate those suffixes. 
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with W. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer** No, the changes made with n_gram_merge is not very different from the key_collision_merge. I think these two are pretty similar in changes made. However, changes were not made in every case, nor are they uniform to the other rows. So it's not technically clean. 
