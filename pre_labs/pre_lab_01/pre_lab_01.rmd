...
In the space below, write the data type for the "id" column and the "seats" column (it's in light gray, offset by \< \>). **Answer** "id" column shows characters and "seats" column represents numerical values. 
...
In the space below, write the name of the business from the first row of the "name" column. **Answer** Directed Research -- but there is also not a name column. We are disregarding this question. 
...
**Answer** Each line of code is ascending based on the term that it is in, in addition with the month it precedes in (in chronological order). Additionally, each of those ascending term items are listed with a count class of how many classes were available during that term. 
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer** Now, instead of the term limits following a chronological descending order, the "count_classes" column is descending in value with the term listed beside it. 
...
**Answer** The data set is arranged based on term and the department. The term is in chronological order, from most earliest to most recent. Additionally, the department names are also listed and arranged in alphabetical order. However, now this time, the "count_class" column is following the arrangements of the terms and departments order. Instead of using: arrange(desc(count_classes)) and only grouping by term, in the code, this codeblock represents grouping by term and department, while using arrange(term, department).  
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer** Now, there has been an addition of a variable called "total_seats" and that will get the sum of the seats in each class. It is not grouped by the "department" column anymore. This codeblock also calls for a descending order of "total_seat" count. Therefore, the terms and count_classes are scattered numbers, but still following the the row correlating to its descending total_seat count. 
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer** There has been additions to the codeblock with the "mean_seats" column and the "median seats" column. Additionally, it is still descending with the "total_seats" column. Now that the median and mean are present, it is easy to see that the top terms have a similar mean number of seats, which is above 44 seats, but within 49 seats, and the median number of seats stay in the 20-25 range. The results are not as affected as the previous codeblocks were from each other. However, it does signify the new additions to the mean and median of the seats in each term, class and the total seats.  
...
**Answer** Now, the data is grouped by department, but arranged by the descending number of the "total_seats" column, which means it'll be going from greatest to least number of total seats in each department. With that, the maximum and minimum number of seats have been added to the codeblock. This now represents a really aggregated dataset, as described in the web book for coding with tidyverse. Not only does it show the descending total seats, mean seats and median seats, but it also shows the minimum and maximum number of seats per department per total seats. 
...
1. How many sections within each couse have seats available? And what is the maximum and minimum number of seats available? 

2. I wonder how many times a course instructor teaches a class or other classes.  

3. Which courses have a syllabus accounted for?  
